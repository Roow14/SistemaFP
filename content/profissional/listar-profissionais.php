...
<div class="janela">
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
			...
			// buscar xxx
			echo '<td>';
			$sqlB = "SELECT * FROM categoria_profissional WHERE id_profissional = '$id_profissional' ";
			$resultB = $conn->query($sqlB);
			if ($resultB->num_rows > 0) {
			    while($rowB = $resultB->fetch_assoc()) {
			...
			echo '<a href="../agenda/agenda-profissional.php?id_profissional='.$id_profissional.'" class="btn btn-default">Agenda</a>';
			echo '<a href="../agenda/agenda-base-profissional.php?id_profissional='.$id_profissional.'" class="btn btn-default">Agenda base</a>';
			echo '</td>';
