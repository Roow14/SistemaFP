...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo '<div class="row profissional">';
        echo '<div class="col-sm-4"><img src="../../vault/profissional/'.$ArquivoMidia.'" style="width:100%;"></div>';
        echo '<div class="col-sm-8">';
        
        if (empty($row['id_profissional'])) {
        } else {
            echo '<label>Nome: </label> <a href="profissional.php?id_profissional='.$id_profissional.'" class="Link"> <b>'.$NomeCompleto.'</b></a><br>';
            echo '<label>Nome social: </label> '.$NomeCurto.'<br>';
        }
        echo '<label>Foto:</label> '.$ArquivoMidia.'<br>';
        if (empty($row['id_profissional'])) {
            echo '<a href="associar-imagem-profissional.php?id_midia_profissional='.$id_midia_profissional.'" class="btn btn-success">Associar profissional</a>';
        } else {
        }
        echo '</div>';
        echo '</div>';
