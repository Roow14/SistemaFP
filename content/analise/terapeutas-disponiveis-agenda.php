...
$FiltroData = 'WHERE agenda_paciente.Data = '.$Data;
...
$FiltroUnidade = 'AND agenda_paciente.id_unidade = '. $id_unidade;
...
<div class="janela">
<h3>Terapeutas disponíveis - Agenda do dia</h3>
    ...
    while($row = $result->fetch_assoc()) {
		...
		if ($resultA->num_rows > 0) {
		    while($rowA = $resultA->fetch_assoc()) {
				...
				// buscar xxx
				$sqlB = "SELECT * FROM agenda_paciente WHERE id_hora = '$id_hora' AND Data = '$Data' AND id_categoria = '$id_categoria' AND id_unidade = '$id_unidade' AND id_profissional = '$id_profissional' ";
				...
				echo '<td><a href="../agenda/agenda-profissional.php?id_profissional='.$id_profissional.'" class="btn btn-default" target="blank">Agenda</a></td>';
				echo '<td><a href="../paciente/paciente.php?id_paciente='.$id_paciente.'" class="Link" target="blank">'.$NomePaciente.'</a></td>';
				if (!empty($id_paciente)) {
					echo '<td><a href="../agenda/agenda-paciente.php?id_paciente='.$id_paciente.'" class="btn btn-default" target="blank">Agenda</a></td>';
				} else {
					echo '<td></td>';
				}
