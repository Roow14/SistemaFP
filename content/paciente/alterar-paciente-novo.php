	...
	<div class="row">
		<form action="alterar-paciente-novo-2.php" method="post" class="form-horizontal">
	        ...
	        <div class="col-sm-12">
			        ...
			        <div class="col-sm-9 ajustetel">
						...
						if ($result->num_rows > 0) {
						    while($row = $result->fetch_assoc()) {
								...
								$NotaTel = $row['NotaTel'];
								echo $NumeroTel.' - '.$NomeClasseTel.' - '.$NotaTel.' <a href="alterar-telefone-novo.php?id_telefone_paciente='.$id_telefone_paciente.'" class="btn btn-default">Alterar</a><br>';
						    }
						} else {
							// não tem
						}
			        ...
			        <div class="col-sm-9 ajustetel">
						...
						if ($result->num_rows > 0) {
						    while($row = $result->fetch_assoc()) {
								...
								echo $EmailPaciente.' - '.$NotaEmail.' <a href="alterar-email-novo.php?id_email_paciente='.$id_email_paciente.'" class="btn btn-default">Alterar</a><br>';
						    }
						} else {
							// não tem
						}
	            ...
	            <div class="form-group">
	                <label class="control-label col-sm-3">Data de início:</label>
	                <div class="col-sm-9">
	               		<input type="date" class="form-control" value="<?php echo $DataInicio;?>" name='DataInicio'>
	               	</div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-9">
	                	<select name="id_processo" class="form-control" required>
		                	<?php
		                	echo '<option value="'.$id_processo.'">'.$Processo.'</option>';
							// buscar xxx
							$sql = "SELECT * FROM processo ORDER BY Processo ASC";
							$result = $conn->query($sql);
							if ($result->num_rows > 0) {
							    while($row = $result->fetch_assoc()) {
									// tem
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-9">
	                	<select name="Status" class="form-control">
		                	<?php
		                	echo '<option value="'.$Status.'">'.$NomeStatus.'</option>';
							?>
