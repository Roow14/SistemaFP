...
<div style="margin-top: 15px;">
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
	    	...
	    	// buscar dados
			$sqlA = "SELECT categoria_profissional.*, profissional.* FROM categoria_profissional LEFT JOIN profissional ON categoria_profissional.id_profissional = profissional.id_profissional WHERE categoria_profissional.id_categoria = '$id_categoria' $FiltroUnidade ORDER BY profissional.NomeCurto ASC";
			$resultA = $conn->query($sqlA);
			if ($resultA->num_rows > 0) {
			    ...
			    while($rowA = $resultA->fetch_assoc()) {
					...
					if ($resultB->num_rows > 0) {
					    while($rowB = $resultB->fetch_assoc()) {
							...
							echo '<a href="../profissional/agenda-profissional.php?id_profissional='.$id_profissional.'" class="Link">'.$NomeCurto.'</a>';
					    }
					} else {
						echo '';
					}
