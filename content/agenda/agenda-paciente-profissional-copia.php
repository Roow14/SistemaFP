...
// buscar xxx
$sqlA = "SELECT * FROM agenda_paciente WHERE id_hora = '$id_hora' AND Data = '$Data' AND id_paciente = '$id_paciente' AND id_unidade = '$id_unidade'";
...
if ($resultA->num_rows > 0) {
    while($rowA = $resultA->fetch_assoc()) {
		...
		if ($resultB->num_rows > 0) {
		    while($rowB = $resultB->fetch_assoc()) {
				...
				echo '<a href="agenda-profissional.php?id_profissional='.$id_profissional.'&id_periodo='.$id_periodo.'&id_unidade='.$id_unidade.'&Data='.$Data.'" class="Link">'.$NomeProfissional.'</a>';
		    }
		} else {
			// não tem
		}
		...
		if (empty($_SESSION['AtivarAlteracaoAtendimento'])) {
			...
			echo '<span style="margin-left: 5px;"><a href="agenda-paciente-remover-profissional.php?id_agenda_paciente='.$id_agenda_paciente.'" class="link-apagar">Cancelar</a></span>';
		}
	...
	$resultB = $conn->query($sqlB);
	if ($resultB->num_rows > 0) {
		echo '<form action="agenda-paciente-profissional-2.php?id_paciente='.$id_paciente.'&Data='.$Data.'&id_unidade='.$id_unidade.'&id_hora='.$id_hora.'&id_periodo='.$id_periodo.'&id_categoria='.$id_categoria.'" method="post" class="">';

	    echo '<div class="input-group" style="margin-left: -15px; margin-right: 15px;">';
		echo '<select class="form-control" name="id_profissionalPesq" required data-toggle="tooltip" title="Profissionais disponíveis">';
	    ...
	    while($rowB = $resultB->fetch_assoc()) {
			...
			if (empty($rowB['id_profissional'])) {
				...
				$sqlA = "SELECT * FROM agenda_paciente WHERE id_profissional = '$id_profissionalPesq' AND Data = '$Data' AND id_unidade ='$id_unidade' AND id_hora = '$id_hora' AND id_periodo = '$id_periodo' AND id_categoria = '$id_categoria'";
				$resultA = $conn->query($sqlA);
				if ($resultA->num_rows > 0) {
