...
$sqlA = "SELECT DISTINCT id_paciente FROM agenda_paciente WHERE id_hora = '$id_hora' AND Data = '$Data' AND id_unidade = '$id_unidade' AND id_categoria = '$id_categoria'";
    ...
    while($rowA = $resultA->fetch_assoc()) {
		...
		if ($resultB->num_rows > 0) {
		    while($rowB = $resultB->fetch_assoc()) {
				...
				echo 'P:<a href="agenda-paciente.php?id_paciente='.$id_pacienteY.'&id_periodo='.$id_periodo.'&id_unidade='.$id_unidade.'&Data='.$Data.'" class="Link"> '.$NomePaciente.'</a><br>';
		    }
		} else {
			// não tem
		}
		...
		// buscar xxx
		$sqlC = "SELECT * FROM agenda_paciente WHERE id_hora = '$id_hora' AND Data = '$Data' AND id_unidade = '$id_unidade' AND id_categoria = '$id_categoria' AND id_paciente = '$id_pacienteY' AND Auxiliar = 2";
		...
		if (empty($_SESSION['AtivarAgendamentoEquo'])) {
			// buscar auxiliar
			$sqlC = "SELECT * FROM agenda_paciente WHERE id_hora = '$id_hora' AND Data = '$Data' AND id_unidade = '$id_unidade' AND id_categoria = '$id_categoria' AND id_paciente = '$id_pacienteY' AND Auxiliar = 1";
			$resultC = $conn->query($sqlC);
			if ($resultC->num_rows > 0) {
			    while($rowC = $resultC->fetch_assoc()) {
					// tem
					$id_agenda_paciente = $rowC['id_agenda_paciente'];
					...
					if ($resultD->num_rows > 0) {
					    while($rowD = $resultD->fetch_assoc()) {
							...
							if (empty($_SESSION['AtivarRemocaoProfissionalEquo'])) {
								...
								echo '<a href="agenda-equo-apagar-profissional-2.php?id_agenda_paciente='.$id_agenda_paciente.'" class="link-apagar">Cancelar</a>';
							}
							echo '<br>';
					    }
					} else {
			...
			if ($resultC->num_rows > 0) {
				...
				$sqlC = "SELECT COUNT(id_profissional) AS Soma FROM agenda_paciente WHERE id_paciente = '$id_pacienteY' AND Data = '$Data' AND id_hora = '$id_hora' AND id_unidade = '$id_unidade' AND id_categoria = '$id_categoria'";
				$resultC = $conn->query($sqlC);
				if ($resultC->num_rows > 0) {
					// tem
					while($rowC = $resultC->fetch_assoc()) {
				...
				if ($Soma < 2) {
					...
					$resultC = $conn->query($sqlC);
					if ($resultC->num_rows > 0) {
						echo '<form action="agenda-equo-paciente-profissional-2.php?id_paciente='.$id_pacienteY.'&Data='.$Data.'&DiaSemana='.$DiaSemana.'&id_unidade='.$id_unidade.'&id_hora='.$id_hora.'&id_periodo='.$id_periodoZ.'&id_categoria='.$id_categoria.'" method="post" class="">';
					    ...
					    while($rowC = $resultC->fetch_assoc()) {
							...
							if (empty($rowC['id_profissional'])) {
								...
								$sqlD = "SELECT * FROM agenda_paciente WHERE id_profissional = '$id_profissionalPesq' AND DiaSemana = '$DiaSemana' AND id_unidade ='$id_unidade' AND id_hora = '$id_hora' AND id_categoria = '$id_categoria'";
								$resultD = $conn->query($sqlD);
								if ($resultD->num_rows > 0) {
								    while($rowD = $resultD->fetch_assoc()) {
										// tem
