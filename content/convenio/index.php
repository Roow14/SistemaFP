...
<form action="index-filtro-2.php" method="post" class="form-inline">
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="Presenca">
        	<option value="<?php echo $Presenca1;?>"><?php echo $NomePresenca1;?></option>
...
$result = $conn->query($sql);
if ($result->num_rows > 0) {
	echo '<form action="validar-convenio-2.php" method="post">';
	// StatusConvenio 1 cancelar validação, 2 validar
	if ((empty($_SESSION['StatusConvenio'])) OR ($_SESSION['StatusConvenio'] == 1)) {
		echo '<input type="text" name="StatusConvenio" value="1" hidden>';
	} else {
		echo '<input type="text" name="StatusConvenio" value="2" hidden>';
	}
    ...
    while($row = $result->fetch_assoc()) {
		...
		if ($resultA->num_rows > 0) {
		    while($rowA = $resultA->fetch_assoc()) {
				// tem
				$NomeConvenioValidado = $rowA['NomeConvenio'];
		    }
		} else {
			// não tem
			$NomeConvenioValidado = NULL;
		}
		...
		echo '<td>'.$NomeConvenioValidado.'</td>';
		echo '<td>';
		echo '<input type="checkbox" name="id_agenda_paciente[]" value="'.$id_agenda_paciente.'">'.$NomeConvenio;
		echo '</td>';
		echo '</tr>';
    }
    echo '</tbody>';
	...
	echo '<b>Nota:</b> Não foi encontrado nenhum paciente agendado para esta data.';
	echo '</div>';
}
...
<div class="modal fade" id="selecionartodos" role="dialog">
    <div class="modal-dialog">
        ...
        <!-- Modal content-->
        <div class="modal-content">
            <form action="validar-convenio-selecionar-todos-2.php" method="post">

                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Validar convênio</h4>
                </div>
                <div class="modal-body" style="background-color: #fafafa;">
                    <select class="form-control" name="Validar" style="margin-bottom: 15px;" required>
                    	<option value="">Selecionar</option>
                    	<option value="1">Validar todos</option>
                    	<option value="2">Cancelar todos</option>
                    </select>
                    <p>Nota: A alteração será aplicada para todos os pacientes que aparecem na lista.</p>
                </div>
