...
<div class="col-sm-12">
	<div style="margin-top: 25px; margin-bottom: 25px;">
	    <?php
		// buscar xxx
		$sql = "SELECT * FROM midia_convenio ORDER BY ArquivoMidia ASC";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
		    while($row = $result->fetch_assoc()) {
				// tem
				$ArquivoMidia = $row['ArquivoMidia'];
				$id_midia_convenio = $row['id_midia_convenio'];
				echo '<p>';
				if (empty($_SESSION['ApagarArquivoAjuda'])) {
				} else {
					echo '<a href="apagar-arquivo-ajuda-2.php?ArquivoMidia='.$ArquivoMidia.'&id_midia_convenio='.$id_midia_convenio.'" class="btn btn-danger">Apagar</a> ';
				}
				echo '<a href="../../vault/convenio/'.$ArquivoMidia.'" class="Link" target="_blank">'.$ArquivoMidia.'</a></p>';
		    }
		} else {
			// não tem
