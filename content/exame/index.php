...
<div class="janela">
    <div style="margin-bottom: 15px;">
        ...
        <!-- <a href="" class="btn btn-default" data-toggle="modal" data-target="#addexame">Adicionar exame</a> -->
    </div>
    <div class="row">
        <div class="col-sm-12">                	
        	<?php
        	// buscar xxx
        	$sql = "SELECT * FROM exame_novo WHERE id_paciente = '$id_paciente' ORDER BY DataExame DESC, id_exame DESC";
        	$result = $conn->query($sql);
        	if ($result->num_rows > 0) {
        	    while($row = $result->fetch_assoc()) {
                    ...
                    if (!empty($TituloExame)) {
                        echo '<label>Titulo:</label> '.$TituloExame.'<br>';
                    }

                    if (!empty($Exame)) {
                        echo '<label>Descrição:</label> '.$Exame;
                    }
